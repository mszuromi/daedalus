"""
models/hawkes_sim_quad_expg_gtas_numba.py
=========================================
Numba-JIT Euler-step simulator for the QUADRATIC Hawkes 2-population
...
The cortical layer is identical to ``hawkes_sim_quad_expg_numba``:
2 neurons with quadratic gain ``phi(v) = a v^2`` and synaptic filter
``g``.  The new piece is the EXTERNAL layer, which is a 2-neuron
GTaS process with its own correlation structure.
...
---------------------------------
    from models.hawkes_sim_quad_expg_gtas_numba import (
        sample_gtas_external_spikes,
        sim_hawkes_quad_expg_gtas_numba,
    )
...
def sim_hawkes_quad_expg_gtas_numba(n_steps, dt_sim, tau, tau_g, a, W_X,
    ...
    """
    Euler-step simulator of the quadratic, exp-filtered Hawkes process
    ...
    for step in range(n_steps):
        ...
        # Draw cortical Poisson spikes: lambda_i = max(a * v_i^2, 0)
        for i in range(npop):
            v_i = v[i]
            lam = a * v_i * v_i
            if lam < 0.0:
