...
Usage (from a SageMath notebook cell)::
    ...
    binned_counts, voltage_bins, total_spikes = sim_hawkes_numba(
...
def sim_hawkes_numba(n_steps, dt_sim, tau, E, W, v_init,
    ...
    voltage_accum = np.zeros(npop)   # accumulated voltage within current bin
    ...
    for step in range(n_steps):
        # Accumulate voltage for the bin average
        ...
        if steps_in_bin >= bin_size_steps:
            # Finalize bin: store mean voltage and advance
            if current_bin < n_bins:
                for i in range(npop):
