"""
api.model_compiler — text expressions → Sage lambdas.

When a ``ModelBuilder`` is fed text expressions (via
...
class _IndexedFormalFunction:
    ...
    Equivalent to ``f[0](...)`` — for models with a single (auto-)
    population, the user can omit the index entirely.  In multi-pop
    models the action text will always carry explicit ``[i]``
    ...
    def __call__(self, *args):
        ...
        # so this branch only fires in single-pop models.
        #
...
class _FieldScalar:
    ...
    fluctuation fields like ``dx`` in scalar (no-population) models
...
def _ns_var_namespace(ns, field_names, param_names, kernel_names,
    ...
    for fname in field_names:
        ...
        # When the field has only one position (scalar model: no
    ...
    # registered (Laplacian only appears for spatial models).
    if hasattr(ns, 'Dt'):
        out['Dt'] = ns.Dt
...
def _build_namespace_for_eval(ns, *, field_names, param_names, kernel_names,
    ...
    # Spatial v2 operator IR: in the ACTION context, when the model opted in
...
class _IndexableCallable:
    ...
    syntax.  In heterogeneous-population models the function body
...
def _lower_operator_ir_action(s, ns, naming_convention):
    ...
    # reducing the action to the validated v1 pipeline.  For a model whose
    ...
    # the model has exactly ONE derivative-vertex type (weight ≡ 1 then).
    if len(vtable) == 1:
...
def make_specializations_lambda(phi_fn_spec: dict | None, *,
    ...
    def _specs(ns):
        ...
        for fn_spec in (functions or []):
            ...
            # single-pop models with size > 1 the two differ —
    ...
    * **compound saddles**: their RHS is a parameter / saddle
    ...
    ``iteration_saddle`` may be:
      ...
      * the sentinel ``'AUTO'`` — every mf_eq whose RHS is a single
        ...
        compound).  Recommended for models with more than one
        iteration saddle.
    """
...
def make_mf_bg_conditions_lambda(mf_eqs: dict[str, str],
    ...
    def _bg(ns):
        ...
        for saddle_name, rhs_text in closure_eqs.items():
            ...
            # heterogeneous-pop models), not ``n_pop`` (which counts
            # how many populations there are).  For single-pop models
