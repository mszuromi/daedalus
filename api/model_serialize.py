"""
api.model_serialize — emit a ``.model.py`` file from a form-
state spec dict.

The model-input UI collects user input as a flat dict; this module
turns that dict into the on-disk format consumed by
``notebooks/model_runner.ipynb`` (and any other loader that imports
...
Output file shape::
    ...
    def build():
        return (
            ModelBuilder(<name>, n_populations=<n>)
            .response_field(...)
...
— useful for the UI's "Load existing model" feature.
"""
...
def _py_repr(value: Any) -> str:
    """``repr`` with sensible formatting for primitive model data.
...
def _emit_field(method: str, f: dict, *, with_natural: bool) -> str:
    ...
    arg is treated as the user-facing natural letter — ModelBuilder
    auto-prefixes ``d`` to get the internal fluctuation name and
    auto-creates the response field + saddle parameter.

    For heterogeneous-population models, ``f['population']`` names
    ...
    # per-field preserves mixed dim=0/dim=d models without needing
...
def _emit_parameter(p: dict) -> str:
    ...
    For heterogeneous-population models, the spec carries an
...
def render_model_file(spec: dict) -> str:
    ...
    # Header
    docstring_lines = [f'"""', f'{name} — text-driven model file.']
    if description:
        docstring_lines += ['', description]
    docstring_lines += [
        '',
        'Generated by the model-input UI '
    ...
    # ModelBuilder shim still loads either (load_spec_from_file accepts all three
    # constructor names).
    _is_spatial = any(int(f.get('spatial_dim', 0) or 0) > 0
                      for f in physical_fields)
    builder_cls = 'SpatialModelBuilder' if _is_spatial else 'TemporalModelBuilder'

    out = []
    out.append('\n'.join(docstring_lines))
    out.append(f'from api.model import {builder_cls}')
    out.append('')
    out.append('')
    ...
    # The new natural-name style relies on ModelBuilder.physical_field
    ...
    # Stability-analysis toggle (default OFF on the ModelBuilder).
    # Only emit when explicitly ON so OFF-by-default models stay
    # textually quiet.  Allows a model file to be diffed cleanly
    # against the UI's intent.
    if spec.get('stability_analysis'):
        out.append('        .stability_analysis(True)')

    # Markovianize default (default ON on the ModelBuilder).  Only
    # emit when explicitly OFF so models that accept the default
    ...
    # model silently loses the Dt()/Lap()/Dx() lowering and computes the
    # wrong physics.  Emitted only when ON so v1 bare-Laplacian models
    ...
    # when present — time-only models carry neither key and stay
    # textually quiet.
    ...
    # non-'off' policy so models on the default stay textually
...
def save_model_to_file(spec: dict, path: str) -> str:
    ...
    if os.path.isdir(path):
        path = os.path.join(path, f'{_slugify(spec["name"])}.model.py')
    elif not path.endswith('.model.py'):
        # User passed a base name — append extension
        if not path.endswith('.py'):
            path = path + '.model.py'

    source = render_model_file(spec)
...
def load_spec_from_file(path: str) -> dict:
    """Parse a ``.model.py`` file's source AST and reconstruct the
    ...
    Used by ``ModelUI.load(path)`` to re-populate the form so the
    user can edit a previously-saved model without rewriting it.
    """
    import ast
    import os
    ...
    # Strip the boilerplate header that render_model_file inserts so
    ...
    spec: dict = {
        ...
        # ``api/colored_to_markovian.py`` and
        ...
        # OFF.  LOAD-BEARING for derivative-vertex spatial models
    ...
    for call in chain:
        # Constructor: ModelBuilder('name', n_populations=N)
        ...
        if method == 'population':
            ...
            # OFF (matches ``ModelBuilder``'s own default).
            if args:
            ...
            # (matches ``ModelBuilder``'s own default).
            if args:
            ...
            # override.)  Mirrors ModelBuilder.spatial_dim semantics.
            if args:
                d = int(args[0])
...
def _strip_doc_boilerplate(doc: str) -> str:
    ...
    :func:`render_model_file` inserts so the user only sees their
    own description on a re-load."""
    if not doc:
        return ''
    # The header pattern is:  "<name> — text-driven model file.\n\n<user>\n\n
    # Generated by the model-input UI ...\nLoaded by ..."
    lines = doc.splitlines()
    # Drop the first line ("<name> — text-driven model file.")
    ...
    keep = []
    for ln in lines:
        if ln.startswith('Generated by the model-input UI'):
            break
