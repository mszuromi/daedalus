...
(t, x) integration (Phase 5):
  ...
  * validation: mixed-dim, bad mode, .boundary on a time-only model
  * non-regression: a time-only model carries none of the new keys

Run::

    sage -python -m pytest tests/test_model_spatial_basics.py -v
"""
...
# ── Builders ─────────────────────────────────────────────────────
def _allen_cahn_1d():
    """The v1 spatial test model (1D subcritical Allen-Cahn)."""
    return (
        ModelBuilder('Allen-Cahn 1D test', n_populations=0)
...
def _time_only_phi4():
    """The same φ⁴ model minus the spatial structure — the
    non-regression control."""
    return (
        ModelBuilder('Time-only phi4 control', n_populations=0)
        .physical_field('phi')
        .parameter('mu',  default=1.0, domain='positive')
        .parameter('lam', default=0.1, domain='positive')
        .parameter('T',   default=1.0, domain='positive')
...
def test_spatial_dim_convenience_bulk_set():
    ...
    b = (ModelBuilder('bulk', n_populations=0)
...
def test_periodic_boundary_named_length():
    m = (ModelBuilder('pbc', n_populations=0)
...
def test_periodic_boundary_inline_length_creates_hidden_param():
    m = (ModelBuilder('pbc-inline', n_populations=0)
         .physical_field('phi', spatial_dim=1)
         .parameter('mu', default=1.0, domain='positive')
         .parameter('D',  default=1.0, domain='positive')
         .parameter('T',  default=1.0, domain='positive')
...
def test_mixed_dim_rejected():
    with pytest.raises(ValueError, match='mixed spatial dimensions'):
        (ModelBuilder('mixed', n_populations=0)
...
def test_boundary_on_time_only_rejected():
    with pytest.raises(ValueError, match='no physical field is spatial'):
        (ModelBuilder('notspatial', n_populations=0)
         .physical_field('phi')
         .parameter('mu', default=1.0)
...
def test_bad_boundary_mode_rejected():
    with pytest.raises(ValueError, match="'infinite' and 'periodic'"):
        (ModelBuilder('badbc', n_populations=0)
         .physical_field('phi', spatial_dim=1)
         .boundary('dirichlet'))
...
def test_bad_initial_mode_rejected():
    with pytest.raises(ValueError, match="only 'stationary'"):
        (ModelBuilder('badic', n_populations=0)
         .physical_field('phi', spatial_dim=1)
         .initial('transient'))
...
def test_periodic_without_length_rejected():
    with pytest.raises(ValueError, match="'length' is required"):
        (ModelBuilder('nolen', n_populations=0)
         .physical_field('phi', spatial_dim=1)
         .boundary('periodic'))
...
# ── Non-regression: time-only model is untouched ─────────────────
def test_time_only_model_has_no_spatial_keys():
    m = _time_only_phi4()
...
def test_serializer_roundtrip_spatial_boundary_initial():
    ...
    with tempfile.NamedTemporaryFile(
            'w', suffix='.model.py', delete=False) as f:
        f.write(src)
        path = f.name
    try:
...
def test_serializer_roundtrip_operator_ir_and_dyson():
    ...
    d = tempfile.mkdtemp()
    path = os.path.join(d, 'rt_opir.model.py')
    save_model_to_file(spec, path)
    ...
    assert '.operator_ir()' in render_model_file(spec2)
...
def test_serializer_spatial_dim_convenience_roundtrip():
    ...
    from api.model_serialize import load_spec_from_file
...
def build():
    return (
        ModelBuilder('conv', n_populations=0)
        .spatial_dim(1)
        .physical_field('phi')
        .parameter('mu', default=1.0, domain='positive')
        .set_action_text('phit*((Dt + mu)*phi)')
...
def _spatial_phi_builder(field_name='phi', extra_param=None):
    ...
    b = (ModelBuilder('rn', n_populations=0)
...
def test_reserved_global_param_t_rejected():
    # 't' is reserved in EVERY model (the Fourier time variable).
    with pytest.raises(ValueError, match=r"'t' is reserved \(all"):
        (ModelBuilder('rn2', n_populations=0)
         .physical_field('phi')
         .parameter('t', default=1.0, domain='positive')
...
def test_x_allowed_in_time_only_model():
    # The conventional time-only field name 'x' must still build.
    model = (ModelBuilder('rn3', n_populations=0)
             .physical_field('x')
             .parameter('mu', default=1.0, domain='positive')
