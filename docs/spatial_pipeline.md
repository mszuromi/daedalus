...
|---|---|---|
| correlator order `k` | `k = 2` (two-point) through `compute_cumulants`; **any `k` at the integrator/bridge level** (June 2026): matrix-`𝓑` Symanzik + multivariate heat-kernel IFT (`_heat_kernel_x_general`) + the orbit–stabilizer external-Wick mapping sum (`diagram_correlator_pts` ÷ `external_wick_compensation`) — plain vertices, any `d`; trust battery in `tests/test_spatial_general_k.py` (k=2 bit-reduction, per-sample `momfac = pref·e^{−q⃗ᵀ𝓑q⃗}` identity vs the independent q-path, analytic IFT vs 2-d quadrature, k=3 route equivalence q-path↔x-path tree+1-loop, permutation invariance, k=3 third-cumulant vs lattice sim) | `k ≥ 3` through the public `compute_cumulants` API (evaluation-point UX); k≥3 derivative-vertex *analytic* IFT (the q-path numerical FT works); k≥3 coupled fields; `method='bessel'` at k≥3 |
