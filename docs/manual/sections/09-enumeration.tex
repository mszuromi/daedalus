...
observed output at the tested orders. The deduplicated \emph{topology} counts ---
$(k,\ell)=(2,1)\to 9$, $(3,1)\to 67$, $(2,2)\to 289$
(\file{loop\_diagram\_enumeration.py:138} comment) --- are the same before and
after the fix.
\end{gotcha}
...
\textbf{9 topologies} --- the anchor number recorded in the code comment at
\file{loop\_diagram\_enumeration.py:138}. Orienting those 9 topologies under the
causality constraints of \S\ref{sec:iso} and deduplicating again gives the final
prediagram list that flows downstream to type assignment. (For reference, the
comment also records $(3,1)\to 67$ and $(2,2)\to 289$ topologies.)
