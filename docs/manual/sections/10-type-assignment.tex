...
time) shrinks. This equivalence is pinned by the regression tests in
\file{tests/test\_type\_assignment.py} --- the \code{test\_leg\_matchings\_*}
family (\code{all\_distinct\_legs}, \code{duplicate\_response\_legs},
\code{mixed\_multiset}, \code{empty\_legs}) and
\code{test\_enumerate\_typed\_distinct\_legs\_regression}.
...
\begin{enumerate}
  ...
  \item \textbf{\code{\_leg\_matchings} emits only the canonical leg ordering.}
        ...
        afterward; pinned by the \code{test\_leg\_matchings\_*} family in
