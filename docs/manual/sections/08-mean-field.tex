...
(\file{pipeline/\_precompute.py:200}), re-runs the solver purely to record the
saddle values for the precompute summary; the separate check that the action's
linear term actually vanishes at the saddle is \code{sanity\_check}, run earlier
at precompute Stage~2 (\file{pipeline/\_precompute.py:141}). The handoff from the saddle to the
