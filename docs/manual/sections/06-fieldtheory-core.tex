...
populations, a synaptic kernel, a formal transfer function --- because
it exercises most of the subsystem's features. (That model multiplies its kernel
image directly; the literal \code{Conv} convolution \emph{operator} of
\S\ref{sec:conv} is exercised not here but by the conductance theory files, which
use the \code{v*Conv(g,n)} form.) Reading that model file alongside
...
of \code{out\_degree} integers from the sources. The prediagram filter
(\file{msrjd/diagrams/filter.py}, which imports and calls \code{available\_degrees})
reads exactly this to discard candidate prediagrams whose vertices would demand a
degree that no available vertex or source can supply --- one screening step of the
enumeration pipeline of Chapter~\ref{ch:enumeration}.
