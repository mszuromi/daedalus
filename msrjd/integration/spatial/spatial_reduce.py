...
form gives the Symanzik matrices
    ...
    M_{ii'} = Σ_e w_e a_{ei} a_{ei'}      (L×L)   — first Symanzik:  U = det M
...
and the integral collapses (``docs/backend_C_math.md`` §2):
    ...
    I_mom = (4πD)^{−Ld/2} · U^{−d/2} · exp[ −D · qᵀ Q_eff q ],
        Q_eff = Q − Nᵀ M⁻¹ N        (the reduced external quadratic form; the
...
def symanzik_matrices(a_list, b_list, weights):
    """Build the Symanzik matrices ``(M, N, Q)`` from per-loop-edge routing
    ...
    Returns ``(M, N, Q)`` as ``np.ndarray`` of shapes ``(L,L)``, ``(L,n_ext)``,
    ``(n_ext,n_ext)``.
    """
...
def symanzik_polynomials(a_list, b_list, weights):
    """C0: return ``(U, Q_eff)`` — the first Symanzik ``U = det M`` and the
    reduced external quadratic form ``Q_eff = Q − Nᵀ M⁻¹ N`` (an ``n_ext×n_ext``
    ...
    """
    M, N, Q = symanzik_matrices(a_list, b_list, weights)
    U = float(np.linalg.det(M))
    if U <= 0.0:
        raise ValueError(f'Symanzik U={U} ≤ 0 — no loop-momentum damping '
                         f'(all Schwinger weights zero?).')
    Q_eff = Q - N.T @ np.linalg.solve(M, N)       # Q − Nᵀ M⁻¹ N
    return U, Q_eff
