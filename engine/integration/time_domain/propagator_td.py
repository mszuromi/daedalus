"""
engine.integration.time_domain.propagator_td
===========================================
...
1. **Fourier convention**:
   ...
   causality filter in `engine.diagrams.causality` guarantees all
...
4. **Index transpose**: the retarded propagator is "response of physical
   ...
   matches `_get_propagator_entry` in `engine/integration/symbolic.py`.
"""
...
def build_G_t_matrix(propagator_data, t_var, num_params=None):
    ...
    propagator_data : dict
        ...
        consumed by `engine.integration.symbolic`.
    t_var : SR variable
...
def G_t_entry(G_t_obj, phys_idx, resp_idx, t_expr,
    ...
    `engine/integration/symbolic.py` line ~305; both paths (Phase I and
