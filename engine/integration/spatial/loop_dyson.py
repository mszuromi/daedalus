...
coupling / combinatorial factor — the caller multiplies by ``M(𝓕)·(coupling)``
...
# uniform-momentum diagram values (Σ_R diagram d[1][0] M(𝓕)=16, Σ_K diagram
# d[1][1] M(𝓕)=8): with the Dyson terms T1 (Σ_R) and T2 (Σ_K) normalized as
...
def bubble_delta_S(q, mu, D, T, g=1.0, formfactor=None):
    ...
    principled weights ``C_R=4, C_K=2`` (from the framework's M(𝓕)).  Even in q.
