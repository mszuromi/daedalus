...
7. EXECUTE the migrated notebook to verify: \`CELL_TIMEOUT=1500 sage -python scratch/exec_nb.py <path>\` (this writes the executed notebook back on success). It MUST run with 0 errors and produce its figures. Confirm the representative cumulant value (e.g. C(tau=0) or C(0,0) or the rate) matches the pre-migration physics (same config => same value).
