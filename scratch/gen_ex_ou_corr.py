...
SIM_CODE = code(
    ...
    "    bins = sim_ou_quartic_two_dim_corr_numba(int(n_steps), float(dt_sim),",
    "            m1, m2, e1, e2, J1, J2, D1, D2, rho, float(x0), float(y0),",
    "            int(bss), int(n_bins), int(1000 + r))       # bins: (2, n_bins) = (x, y)",
    "    # CROSS slice: leg 0 → x (index 0), leg 1 swept → y (index 1).",
    "    _, C = compute_kpoint_slice(binned_counts=np.zeros_like(bins),",
    "            dt_bin=float(dt_bin_eff), pop_indices=[0, 1], lag_bins=[0, None],",
    "            max_lag_bins=int(max_lag_bins), field_types=['dv', 'dv'],",
    "            voltage_bins=bins)",
    "    C_runs.append(np.asarray(C).real)",
    "C_runs = np.array(C_runs)",
    "sim = {'tau': tau_sim, 'C': C_runs.mean(0),",
    "       'C_err': C_runs.std(0, ddof=1) / np.sqrt(N_RUNS)}",
    "",
    "mid = len(res['tau_grid']) // 2",
    "s0 = len(sim['C']) // 2",
    ...
    "print('C_xy(0)  simulation         = %+.4f  (+/- %.4f)' % (sim['C'][s0], sim['C_err'][s0]))",
