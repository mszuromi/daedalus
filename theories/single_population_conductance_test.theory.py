"""
Single-population CONDUCTANCE-style synapse — text-driven theory file.

Action term: ``- w[i,j] * v[i] * Conv(g[i,j], n[j])``  (the ``v[i]``
voltage outside the convolution turns the synapse into a conductance,
producing an extra interaction vertex where the kernel ``g`` attaches
to the n-leg).
