"""
Single population Conductance Synapse Test — text-driven theory file.
...
def build():
    return (
        TemporalTheoryBuilder('Single population Conductance Synapse Test')
    ...
    [0.2, 0.0],
        ...
        .set_action_text('''
            ...
            - sum(w[i,j]*(Eg - v[i])*Conv(g,n[j]) for j in E))
            for i in E)
        ''')
        .set_mf_equation('vstar', '((Em[i] + Eg*sum(w[i,j]*nstar[j] for j in E)) / (1 + sum(w[i,j]*nstar[j] for j in E)))')
