...
def build():
    return (
        TemporalTheoryBuilder('Single population Quad Spike Reset Test')
    ...
    [0.2, 0.3],
        ...
        .set_action_text('''
            ...
            + vt[i]*((tau[i]*Dt + 1)*v[i] - Em[i] + (1/2)*v[i]*n[i]
            - sum(w[i,j]*Conv(g,n[j]) for j in E))
            for i in E)
        ''')
        .set_mf_equation('vstar', '(Em[i] + sum(w[i, j]*nstar[j] for j in E)) / (1 + (1/2)*nstar[i])')
